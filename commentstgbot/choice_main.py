...
from commentstgbot.config import config
from commentstgbot.config.config import Config, load_arg_yaml

parser = argparse.ArgumentParser(description="config_file")
parser.add_argument("-f", type=str)
args = parser.parse_args()
logger.success(args)
config.config = Config(**load_arg_yaml(args.f))
...
async def main():
    ...
    init_logging(filename=args.f, old_logger=True, level="TRACE", old_level=logging.INFO, steaming=False, write=True)
